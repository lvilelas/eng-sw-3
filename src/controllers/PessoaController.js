...
class PessoaController{
    async cadastro(req,res) {
        ...
        try {
            let endereco = await Endereco.findOne({"cep":req.body.pessoa.endereco.cep});
            if(endereco == null){
                endereco = await Endereco.create(req.body.pessoa.endereco);
            }
            req.body.pessoa.endereco = endereco;
    ...
    async buscaPorDoc(req,res){
        if(req.params.tipo == "J"){
            ...
            const pj = await PJ.findOne({"cnpj":cnpj}).populate("pessoa");
            return (pj==null) ? res.status(200).json({"mensagem":"Usuario não cadastrado"}) : res.json(pj);
        }else if(req.params.tipo == "F"){
            var cpf = req.params.doc;
            cpf = cpf.replace(/^([\d]{3})([\d]{3})([\d]{3})([\d]{2})$/, "$1.$2.$3-$4");
            const pf = await PF.findOne({"cpf":cpf}).populate("pessoa").populate("endereco");
